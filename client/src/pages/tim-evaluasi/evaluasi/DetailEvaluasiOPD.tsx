...
import { useEvaluasiDraft } from "@/api/evaluasi";
import {
  useEvaluasiSopByOpd,
  useRiwayatEvaluasiSop,
  useRiwayatEvaluasiOpd,
} from "@/api/evaluasi";
import { useEvaluasiSubmit } from "@/api/evaluasi";
...
export function DetailEvaluasiOPD() {
  const { id: opdId } = useParams({ from: "/tim-evaluasi/evaluasi/$id" });
  const { sopId: preferredSopId } = useSearch({
    from: "/tim-evaluasi/evaluasi/$id",
  });
  ...
  /* Jaga selectedSopId konsisten saat filter status/evaluator berubah (bukan fetch). */
  useEffect(() => {
  ...
  /* Saat dialog submit dibuka, centang semua SOP "sedang dievaluasi" di daftar (bukan fetch). */
  useEffect(() => {
