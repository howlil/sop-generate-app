...
import { SOPStatusFilterSelect, BuatSOPDialog, sopApi } from "@/features/sop";
import { usePeraturan } from "@/features/organisasi";
import {
  canEditSop,
  canTimPenyusunRunCoordinatorActions,
  useSop,
  useDaftarSopFilters,
  useDaftarSopData,
} from "@/features/sop";
...
export function ManajemenSOP() {
  ...
  const { isLoading, error, create } = useSop();
  ...
  const confirmAjukanEvaluasiBulk = async () => {
    if (!canTimPenyusunRunCoordinatorActions(role ?? "")) {
      showToast(
        "Hanya Koordinator Tim Penyusun yang dapat mengajukan evaluasi.",
        "error",
      );
      return;
    }
    ...
    <ListPageLayout
      ...
      toolbar={
        ...
        >
          ...
          {canTimPenyusunRunCoordinatorActions(role ?? "") && (
            <Button
              variant="outline"
              size="sm"
              className="h-8 text-xs gap-1.5"
                          ...
                          <div>
                            ...
                            {sop.lastEditedAt && (
                              <p className="text-gray-400 text-xs mt-0.5">
                                {formatDateIdLong(sop.lastEditedAt)}
                              </p>
                            )}
                          </div>
                        ) : (
