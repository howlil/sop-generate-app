...
import type { KredensialTTE, RegisterTteDto, TTERole } from "@/types/dto/tte.dto";
...
export interface TTEBuatDialogProps {
  ...
  defaultNip: string;
  defaultNama: string;
  profile?: KredensialTTE;
  onRegisterTTE: (payload: RegisterTteDto) => Promise<unknown>;
  onMintTokenVerifikasi: () => Promise<{ token: string }>;
...
export function TTEBuatDialog({
  ...
  const handleNextFromPin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const tokenResult = await onMintTokenVerifikasi();
