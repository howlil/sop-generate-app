...
export function rowsToSteps(
  ...
  return rows.map((row) => {
    const pelaksanaId =
      typeof row.pelaksana === 'string' ? row.pelaksana.trim() : ''
    const mapping = row.pelaksanaMapping ?? {}
    const mappingImplementerId = Object.keys(mapping).find(
      (key) => mapping[key] && mapping[key] !== ''
    )
    /* Sumber kebenaran lane: `row.pelaksana` (editor/API terbaru).
       `pelaksanaMapping` hanya fallback legacy agar kompatibel data lama. */
    const implementerId =
      pelaksanaId.length > 0
        ? pelaksanaId
        : (mappingImplementerId ?? implementers[0]?.id)
    const type =
      row.type ??
