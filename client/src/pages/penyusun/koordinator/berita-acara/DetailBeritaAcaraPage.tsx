...
import { useEvaluasiDetail } from "@/api/evaluasi";
...
const STATUS_LABEL: Record<string, string> = {
  ...
  DITANDATANGANI_KOORDINATOR: 'Ditandatangani PJ Penyusun',
  DISETUJAI: 'Disetujui',
  DITOLAK: 'Ditolak',
}
...
export function DetailBeritaAcaraPage() {
  const { id } = useParams({ from: '/penyusun/koordinator/berita-acara/$id' })
  ...
  return (
    <>
      <DetailPageLayout
        ...
        { label: 'Berita Acara', to: ROUTES.PENYUSUN.KOORDINATOR_BERITA_ACARA },
        { label: pengajuan?.opdNama ?? 'Detail' },
      ]}
      title="Detail Berita Acara"
      description={pengajuan?.opdNama ?? 'Memuat...'}
      backTo={ROUTES.PENYUSUN.KOORDINATOR_BERITA_ACARA}
      backSize="icon"
      actions={
        isReadyForSignature && (
          <Button
        ...
        <>
          ...
          {error && (
            <div className="flex flex-col items-center justify-center py-12 text-center">
              ...
              <Button
                variant="outline"
                onClick={() => navigate({ to: ROUTES.PENYUSUN.KOORDINATOR_BERITA_ACARA })}
              >
                Kembali ke Daftar
              </Button>
            </div>
              ...
              <TabsContent value="berita-acara" className="mt-6">
                <div className="rounded-lg border bg-gray-50 p-6">
                  <BeritaAcaraTemplate
                    ...
                    tteSignaturePayload={parseTTESignaturePayload(
                      pengajuan.tteSignaturePayload,
                    )}
                  />
                </div>
              </TabsContent>
