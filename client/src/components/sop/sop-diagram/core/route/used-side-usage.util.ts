import type { PathUpdatedPayload } from '../../shapes/FlowchartArrowConnector'
import type { UsedSides } from './selectSidePairs'

const SIDE_KEYS: Array<PathUpdatedPayload['sSide']> = ['top', 'bottom', 'left', 'right']

function cloneDirection(
  dir: UsedSides[string]['in'] | UsedSides[string]['out'] | undefined,
): Partial<Record<PathUpdatedPayload['sSide'], string[]>> {
  const next: Partial<Record<PathUpdatedPayload['sSide'], string[]>> = {}
...
function stripConnection(
  dir: Partial<Record<PathUpdatedPayload['sSide'], string[]>>,
  connectionId: string,
): void {
  for (const side of SIDE_KEYS) {
    const values = dir[side]
...
export function applyUsedSidePayload(prev: UsedSides, payload: PathUpdatedPayload): UsedSides {
