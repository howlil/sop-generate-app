...
function HeaderPage({
  ...
  const revisionDate = Number(metadata.version ?? 1) > 1 ? headerDisplayDate(metadata.revisionDate) : ''
          ...
          <View style={styles.row}>
            ...
            <View style={{ width: '55%' }}>
              ...
              <View style={styles.row}>
                ...
                <View style={[styles.cellTight, { width: '48%', minHeight: 82, alignItems: 'center' }]}>
                  {qrDataUrlKepalaOpd ? (
  ...
  if (type === 'terminator') {
    return (
      <Svg width={34} height={18} viewBox="0 0 34 18">
        <Rect x={1} y={1} width={32} height={16} rx={8} ry={8} fill="none" stroke="#000000" strokeWidth={1.2} />
      </Svg>
    )
  }
  return (
    ...
    <Page size={A4_LANDSCAPE} orientation="landscape" style={styles.centeredPage}>
      ...
      <View style={styles.table}>
        <View style={styles.row}>
          ...
          <View style={[styles.stepHeaderCell, { width: '24%' }]}>
            <Text style={[styles.label, styles.centerText]}>KEGIATAN</Text>
          </View>
          <View style={[styles.stepHeaderCell, { width: `${pelaksanaWidth}%` }]}>
            <Text style={[styles.label, styles.centerText]}>PELAKSANA</Text>
          </View>
          <View style={[styles.stepHeaderCell, { width: '14%' }]}>
            <Text style={[styles.label, styles.centerText]}>KELENGKAPAN</Text>
          </View>
          <View style={[styles.stepHeaderCell, { width: '8%' }]}>
            <Text style={[styles.label, styles.centerText]}>WAKTU</Text>
          </View>
          <View style={[styles.stepHeaderCell, { width: '13%' }]}>
            <Text style={[styles.label, styles.centerText]}>OUTPUT</Text>
          </View>
          <View style={[styles.stepHeaderCell, { width: '12%' }]}>
            <Text style={[styles.label, styles.centerText]}>KET</Text>
          </View>
        </View>
        <View style={styles.row}>
          <View style={[styles.stepHeaderCell, { width: '29%' }]} />
          {safeImplementers.map((impl) => (
            <View key={impl.id} style={[styles.stepHeaderCell, { width: `${implWidth}%` }]}>
              <Text style={[styles.label, styles.centerText, { fontSize: 6 }]}>{impl.name.toUpperCase()}</Text>
            </View>
          ))}
          <View style={[styles.stepHeaderCell, { width: '47%' }]} />
        </View>
        {rows.map((row) => (
          <View key={row.id} style={styles.row} wrap={false}>
            ...
            <View style={[styles.stepBodyCell, { width: '14%' }]}>
              <Text style={[styles.justifyText, styles.stepText]}>{row.mutu_kelengkapan || row.kelengkapan || ' - '}</Text>
            </View>
            ...
            <View style={[styles.stepBodyCell, { width: '13%' }]}>
              <Text style={[styles.justifyText, styles.stepText]}>{row.output || row.keluaran || ' - '}</Text>
            </View>
