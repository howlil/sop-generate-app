...
export interface UseProsedurEditorReturn {
  ...
  // Row operations
  handleAddRow: (index: number, implementers: { id: string; name: string }[]) => void
  handleDeleteRow: (index: number) => void
  handleTypeChange: (index: number, type: ProsedurRow['type']) => void
  handleKegiatanChange: (index: number, kegiatan: string) => void
  handlePelaksanaChange: (index: number, implementerId: string, implementers: { id: string; name: string }[]) => void
...
export function useProsedurEditor(
  ...
  // Row operations
  const handleAddRow = useCallback((index: number, implementers: { id: string; name: string }[]) => {
  ...
  const handleKegiatanChange = useCallback((index: number, kegiatan: string) => {
    setProsedurRows((prev) =>
      prev.map((r, i) => (i === index ? { ...r, kegiatan } : r))
    )
  }, [setProsedurRows])

  const handlePelaksanaChange = useCallback((index: number, implementerId: string, implementers: { id: string; name: string }[]) => {
    setProsedurRows((prev) =>
      prev.map((r, i) => {
        if (i !== index) return r
  ...
  const handleMutuKelengkapanChange = useCallback((index: number, value: string) => {
    setProsedurRows((prev) =>
      prev.map((r, i) => (i === index ? { ...r, mutu_kelengkapan: value } : r))
    )
  }, [setProsedurRows])

  const handleMutuWaktuChange = useCallback((index: number, amount: string, unit: string) => {
  ...
  const handleKeteranganChange = useCallback((index: number, value: string) => {
    setProsedurRows((prev) =>
      prev.map((r, i) => (i === index ? { ...r, keterangan: value } : r))
    )
  }, [setProsedurRows])

  const handleDecisionConfig = useCallback((index: number, yesId: string, noId: string) => {
