...
describe('Pengujian PengajuanEvaluasiService', () => {
  ...
  it.each([JenisPengajuanEvaluasi.TERJADWAL, JenisPengajuanEvaluasi.MANDIRI])(
    'seharusnya menyimpan jenis %s saat membuat pengajuan',
    async (jenis) => {
  ...
  it('seharusnya menolak pembuatan ketika status detail belum siap dievaluasi', async () => {
    const opdId = '00000000-0000-4000-8000-000000000001';
    const detailSopId = '00000000-0000-4000-8000-000000000002';
    const tx = {
      pengajuanEvaluasi: {
  ...
  it('seharusnya melempar ConflictException ketika status berubah sebelum dipromosikan', async () => {
    ...
    const tx = {
      ...
      detailSOP: {
        findMany: jest.fn().mockResolvedValue([{ detailSopId, status: StatusSOP.SIAP_DIEVALUASI }]),
  ...
  it('seharusnya tidak melakukan perubahan pada evaluasi mandiri ketika pengguna bukan evaluator', async () => {
    const runTransaction = jest.fn();
    const repo = { runTransaction } as unknown as PengajuanEvaluasiRepository;
    const service = buildService(repo);
      ...
      [
        {
          ...
          statusDetail: StatusSOP.SIAP_DIEVALUASI,
        },
      ],
    );
    expect(runTransaction).not.toHaveBeenCalled();
  ...
  it('seharusnya tidak melakukan perubahan pada evaluasi mandiri ketika alur pengajuan tidak memenuhi syarat', async () => {
    const runTransaction = jest.fn();
    const repo = { runTransaction } as unknown as PengajuanEvaluasiRepository;
    const service = buildService(repo);
    await service.pastikanPengajuanMandiriUntukEvaluator(
      ...
      [
        {
          detailSopId: '00000000-0000-4000-8000-000000000002',
          statusDetail: StatusSOP.SIAP_DIVERIFIKASI,
        },
      ],
    );
    expect(runTransaction).not.toHaveBeenCalled();
  ...
  it('seharusnya tidak melakukan perubahan pada evaluasi mandiri ketika status sudah diajukan evaluasi', async () => {
    const runTransaction = jest.fn();
    const repo = { runTransaction } as unknown as PengajuanEvaluasiRepository;
    const service = buildService(repo);
    await service.pastikanPengajuanMandiriUntukEvaluator(
  ...
  it('seharusnya membuat evaluasi mandiri dalam transaksi ketika tidak ada penghambat', async () => {
    const detailSopId = '00000000-0000-4000-8000-000000000002';
    const tx = {
      pengajuanEvaluasi: {
        findFirst: jest.fn().mockResolvedValue(null),
  ...
  describe('findOne (Tambahan Kasus)', () => {
    it('seharusnya melempar NotFoundException jika pengajuanEvaluasiId tidak ditemukan (False Case)', async () => {
      ...
      const service = buildService(repo);
      await expect(service.findOne({ sub: 'u', email: 'e', peran: PeranPengguna.PJ_PENYUSUN }, 'invalid')).rejects.toBeInstanceOf(NotFoundException);
    });
    ...
    it('seharusnya melempar ForbiddenException jika mencoba mengakses pengajuan dari OPD lain (False Case)', async () => {
      ...
      const assertSameOpd = jest.fn().mockRejectedValue(new ForbiddenException('Anda tidak dapat mengakses pengajuan evaluasi OPD lain'));
      const service = buildService(repo, { assertSameOpd });
      await expect(service.findOne({ sub: 'u', email: 'e', peran: PeranPengguna.PENYUSUN }, 'p1')).rejects.toBeInstanceOf(ForbiddenException);
    });
    ...
    it('seharusnya mengembalikan data ringkas jika ditemukan dan terotorisasi (Success Case)', async () => {
      ...
      const service = buildService(repo, { assertSameOpd });
      const result = await service.findOne({ sub: 'u', email: 'e', peran: PeranPengguna.PENYUSUN }, 'p1');
  ...
  describe('create (Tambahan Kasus)', () => {
    it('seharusnya melempar BadRequestException jika sopDetailIds mengandung duplikasi (Worst/Edge Case)', async () => {
      ...
      const getRequiredUserOpdId = jest.fn().mockResolvedValue('opd-a');
      const service = buildService(repo, { getRequiredUserOpdId });
      await expect(service.create(userPjPenyusun, {
        jenis: JenisPengajuanEvaluasi.TERJADWAL,
    ...
    it('seharusnya melempar ConflictException jika data langsung hilang sesaat sesudah transaksi dibuat (False Case)', async () => {
      const tx = {
        ...
        detailSOP: {
          findMany: jest.fn().mockResolvedValue([{ detailSopId: 'id1', status: StatusSOP.SIAP_DIEVALUASI }]),
          updateMany: jest.fn().mockResolvedValue({ count: 1 })
        }
      };
      const runTransaction = jest.fn(async (cb: (t: typeof tx) => Promise<string>) => cb(tx));
      const repo = { runTransaction, findByIdFull: jest.fn().mockResolvedValue(null) } as unknown as PengajuanEvaluasiRepository;
      const getRequiredUserOpdId = jest.fn().mockResolvedValue('opd-a');
      const service = buildService(repo, { getRequiredUserOpdId });
      await expect(service.create(userPjPenyusun, {
        jenis: JenisPengajuanEvaluasi.TERJADWAL,
  ...
  describe('pastikanPengajuanMandiriUntukEvaluator (Tambahan Kasus)', () => {
    ...
    it('seharusnya melempar ConflictException jika baris diupdate tidak cocok (promoted.count !== sopDetailIds.length) di transaksi (False Case)', async () => {
      ...
      const repo = { runTransaction } as unknown as PengajuanEvaluasiRepository;
      const service = buildService(repo);
      await expect(service.pastikanPengajuanMandiriUntukEvaluator(
        { sub: 'eval', email: 'e', peran: PeranPengguna.EVALUATOR },
        'opd-a',
        [{ detailSopId: 'id1', statusDetail: StatusSOP.SIAP_DIEVALUASI }]
      )).rejects.toBeInstanceOf(ConflictException);
    });
  });
  ...
  describe('Otorisasi Akses: resolveOpdIdTerikat & assertUserCanAccessPengajuan (Tambahan Kasus)', () => {
    it('resolveOpdIdTerikat seharusnya melempar ForbiddenException jika user adalah EVALUATOR (karena global) (False Case)', async () => {
      const repo = {} as unknown as PengajuanEvaluasiRepository;
      const service = buildService(repo);
      await expect(service.resolveOpdIdTerikat({ sub: 'eval', email: 'e', peran: PeranPengguna.EVALUATOR })).rejects.toBeInstanceOf(ForbiddenException);
    });
    ...
    it('assertUserCanAccessPengajuan seharusnya me-resolve jika user adalah EVALUATOR tanpa cek OPD (Edge Case)', async () => {
      ...
      const service = buildService(repo, { assertSameOpd });
      await expect(service.assertUserCanAccessPengajuan({ sub: 'eval', email: 'e', peran: PeranPengguna.EVALUATOR }, 'opd-a')).resolves.toBeUndefined();
    ...
    it('assertUserCanAccessPengajuan seharusnya melempar ForbiddenException untuk role yang tidak dikenal (False Case)', async () => {
      const repo = {} as unknown as PengajuanEvaluasiRepository;
      const service = buildService(repo);
      await expect(service.assertUserCanAccessPengajuan({ sub: 'admin', email: 'a', peran: 'ADMIN' as PeranPengguna }, 'opd-a')).rejects.toBeInstanceOf(ForbiddenException);
    });
  });
});
