...
export class EvaluasiUmpanBalikService {
  ...
  async getUmpanBalikForDetail(
    ...
    const detailOpdId = await this.evaluasiNilaiRepository.findOpdIdByDetailSopId(detailSopId);
    if (detailOpdId === null) {
    ...
    const row = await this.evaluasiNilaiRepository.findUmpanBalikForDetail(detailSopId, opdId);
    if (row === null) {
      return null;
    }
