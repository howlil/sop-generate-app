...
describe('Pengujian OpdService', () => {
  ...
  describe('listRingkas', () => {
    it('seharusnya mengembalikan seluruh opd untuk peran PJ_EVALUATOR', async () => {
      ...
      const result = await service.listRingkas({ sub: 'u1', email: 'u@test.com', peran: PeranPengguna.PJ_EVALUATOR }, 'cari');
    ...
    it('seharusnya mengembalikan [] ketika pengguna bukan PJ_EVALUATOR dan belum terhubung dengan OPD manapun (Worst case)', async () => {
      ...
      const result = await service.listRingkas({ sub: 'u2', email: 'u2@test.com', peran: PeranPengguna.PENYUSUN });
    ...
    it('seharusnya mengembalikan [] ketika OPD pengguna tidak ditemukan atau tidak aktif (Edge case)', async () => {
      ...
      const result = await service.listRingkas({ sub: 'u3', email: 'u3@test.com', peran: PeranPengguna.PENYUSUN });
      expect(result).toEqual([]);
    });

    it('seharusnya mengembalikan OPD milik pengguna ketika ditemukan', async () => {
      opdRepository.findOpdIdByPenggunaId.mockResolvedValue('opd-1');
      opdRepository.findRingkasAktifById.mockResolvedValue({ opdId: 'opd-1', nama: 'OPD Pengguna' });
      const result = await service.listRingkas({ sub: 'u4', email: 'u4@test.com', peran: PeranPengguna.PENYUSUN });
  ...
  describe('create', () => {
    it('seharusnya dapat membuat OPD baru dan membersihkan spasi', async () => {
      const date = new Date();
      opdRepository.create.mockResolvedValue({ opdId: 'opd-new', nama: 'OPD Baru', createdAt: date, updatedAt: date, deletedAt: null });
  ...
  describe('update', () => {
    it('seharusnya melempar NotFoundException jika OPD tidak ditemukan (False case)', async () => {
      ...
      await expect(service.update('tidak-ada', { nama: 'Test' })).rejects.toBeInstanceOf(NotFoundException);
    });

    it('seharusnya memperbarui nama OPD dan memotong spasi', async () => {
      const date = new Date();
      opdRepository.findAktifById.mockResolvedValue({ opdId: 'opd-1', nama: 'Lama', createdAt: date, updatedAt: date, deletedAt: null });
      opdRepository.update.mockResolvedValue({ opdId: 'opd-1', nama: 'Baru', createdAt: date, updatedAt: date, deletedAt: null });
